...
public class CurrencyTest extends TestCase {
    ...
    public void testInitCurrencies(){
        int i = 0;
        List<Currency> actual = currency.initCurrencies();

        List<Currency> excepted = currencies;

        for (Currency currency1 : actual){
            for (Currency currency2 : excepted){
                if(currency1.getName().contains(currency2.getName())
                && currency1.getShortName().contains(currency2.getShortName())){
                    ++i;
                }
            }
        }
        ...
        assertEquals(excepted.size(), i);
    }
    public void testConvert(){
        double excepted = 100.0 * exchangeValues.get("EUR");
        excepted = Math.round(excepted * 100d) / 100d;

        double actual = Currency.convert(100.0, exchangeValues.get("EUR"));
    ...
    public void testInitValues(){
        ...
        for (int i = 0; i < currencies.size(); i++){
            ...
            for (Map.Entry<String, Double> entry : exchangeValues.entrySet()){
                currency.setExchangeValues(entry.getKey(), entry.getValue());
            }
        }

        Map<String, Double> actual = currency.getExchangeValues();
        Map<String, Double> excepted = exchangeValues;

        for (Map.Entry<String, Double> entry : actual.entrySet()){
            for (Map.Entry<String, Double> subEntry : excepted.entrySet()){
                if (entry.getKey().contains(subEntry.getKey())
                        &&
                        (entry.getValue().compareTo(subEntry.getValue()) == 0)){
                    ++temp;
                }
            }
        }
        assertEquals(excepted.size(), temp);
    }
}
